...
func (s *Service) CreateNewMetric(cmm *MetricMetadata) error {
	insertIntoMetricsResult, err := s.context.Db.Exec(`INSERT INTO metrics (metric_id , description , title ) VALUES ($1, $2, $3)`, cmm.MetricId, cmm.Description, cmm.Title)
	...
	for _, metricQuery := range cmm.Queries {
		...
		insertIntoMetricsQueriesResult, err := s.context.Db.Exec(`INSERT INTO metrics_queries (metric_id ,query_id ) VALUES ($1, $2)`, cmm.MetricId, queryID)
...
func (s *Service) getMetricMetadata() ([]*MetricMetadata, error) {
	var metricsModel []*MetricMetadata

	err := s.context.Db.Select(&metricsModel, `SELECT m.metric_id, m.description, m.title FROM metrics m`)
...
func (s *Service) GetMetrics(tenantId int32, queryModel *GetMetricsRequestModel) []*MetricsEntity {
	...
	var metricsMetadata []*MetricMetadata

	golog.Infof(" The Metrics Are: %s ", strings.Join(queryModel.Metrics, ","))

	err := s.context.Db.Select(&metricsMetadata,
		`SELECT m.metric_id, m.title, m.description, array_to_json(array_agg(row_to_json(q.*))) as queries
		...
		strings.Join(queryModel.Metrics, ","))
	...
	for _, metricMetadata := range metricsMetadata {
		metricEntity := s.getMetric(metricMetadata, queryModel, tenantId)
...
func NewMetricsEntity(mm *MetricMetadata) *MetricsEntity {
	golog.Info("NewMetricsEntity")
