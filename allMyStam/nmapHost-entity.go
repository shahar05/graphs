// This file is take and modified from https://github.com/Ullaakut/nmap/blob/master/xml.go
package entities

import (
	"eagle-server/agentif/agent"
	"encoding/xml"
	"fmt"
	"io/ioutil"
	"regexp"
...
// matching null representations: \000, \0, \x00, \u0000, \z
var isNull = regexp.MustCompile(`\\0{3}|\\0{1}|\\x00|\\u0000|\\z`)

// NMapRunEntity represents an nmap scanning run.
type NMapRunEntity struct {
...
// ToFile writes a NMapRunEntity as XML into the specified file path.
func (r NMapRunEntity) ToFile(filePath string) error {
...
type Host struct {
	Distance *Distance `xml:"distance" json:"distance,omitempty"`
	// EndTime  *Timestamp `xml:"endtime,attr,omitempty" json:"end_time,omitempty"`
	IPIDSequence *IPIDSequence `xml:"ipidsequence" json:"ip_id_sequence,omitempty"`
	OS           *OS           `xml:"os" json:"os,omitempty"`
	// StartTime    *Timestamp    `xml:"starttime,attr,omitempty" json:"start_time,omitempty"`
	Status        *Status        `xml:"status" json:"status,omitempty"`
	TCPSequence   *TCPSequence   `xml:"tcpsequence" json:"tcp_sequence,omitempty"`
	TCPTSSequence *TCPTSSequence `xml:"tcptssequence" json:"tcp_ts_sequence,omitempty"`
	Times         *Times         `xml:"times" json:"times,omitempty"`
	Trace         *Trace         `xml:"trace" json:"trace,omitempty"`
	Uptime        *Uptime        `xml:"uptime" json:"uptime,omitempty"`
	...
	Hostnames     []Hostname     `xml:"hostnames>hostname" json:"hostnames,omitempty"`
...
type EditModel struct {
	HostName string          `json:"hostName" json:"host_name"`
	IP       int             `json:"ip" json:"ip"`
	Type     string          `json:"type" json:"type"`
	Class    string          `json:"class" json:"classification"`
	OsFamily string          `json:"osFamily" json:"os_family"`
	OsName   string          `json:"osName" json:"os_name"`
	OsCPE    string          `json:"osCpe" json:"os_cpe"`
	Ports    []EditPortModel `json:"ports" json:"ports"`
}

type EditPortModel struct {
	Port    int    `json:"port" json:"port"`
	Service string `json:"service" json:"service"`
	Product string `json:"product" json:"product"`
	Info    string `json:"info" json:"info"`
	CPE     string `json:"cpe" json:"cpe"`
}

// ExtractHostName from nmap payload
func (h *Host) ExtractHostName() string {
	netBiosName := ""
	fqdnName := ""
	for _, script := range h.HostScripts {
		if script.ID == "smb-os-discovery" {
			list := strings.Split(script.Output, "\n")
			for _, s := range list {
				str := strings.Split(s, "Computer name: ")
				if len(str) == 2 {
					nameTemp := removeNull(str[1])
					if nameTemp != "" {
						// if computer name found no need to continue
						return nameTemp
					}
				}

				str = strings.Split(s, "FQDN: ")
				if len(str) == 2 {
					nameTemp := removeNull(str[1])
					if nameTemp != "" {
						fqdnName = nameTemp
					...
					str = strings.Split(s, "NetBIOS computer name: ")
					if len(str) == 2 {
						nameTemp := removeNull(str[1])
						if nameTemp != "" {
							netBiosName = nameTemp
						}
					}
				}
			}
		} else if script.ID == "nbstat" && fqdnName == "" {
			list := strings.Split(script.Output, "\n")
			for _, s := range list {
				str := strings.Split(s, "NetBIOS name: ")
				if len(str) == 2 {
					name := strings.Split(str[1], ",")
					if len(name) >= 1 {
						nameTemp := removeNull(name[0])
						if nameTemp != "" {
							netBiosName = nameTemp
						}
					}
	...
	for _, p := range h.Ports {
		for _, s := range p.Scripts {
			if s.ID == "rdp-ntlm-info" {
				for _, e := range s.Elements {
					if e.Key == "DNS_Computer_Name" {
						nameTemp := removeNull(e.Value)
						if nameTemp != "" {
							return nameTemp
						}
...
// GetMACAddress searches for MAC Address, returns error if missing
func (h *Host) GetMACAddress() (string, error) {
	...
	return "", fmt.Errorf("GetMACAddress: Host doesn't have mac address")
}

// GetHostname searches for the first hostname, returns empty string if missing
func (h *Host) GetHostname() string {
	if len(h.Hostnames) > 0 {
		return removeNull(h.Hostnames[0].Name)
	}

	return ""
}

// GetOSType converting os family to os type
func (h *Host) GetOSType() agent.OSType {
	if h.OS != nil && len(h.OS.Matches) > 0 && len(h.OS.Matches[0].Classes) > 0 {
		osName := strings.ToLower(string(h.OS.Matches[0].Classes[0].OSFamily()))
		return oSNameToType(osName)
	}

	return agent.OSType_OSNull
}

// GetOSType converting os family to os type
func (h *Host) GetOSName() string {
...
// GetIPv4 searches for the ipv4, returns err if missing
func (h *Host) GetIPv4() (string, error) {
	const nmapAddrIPv4Field = "ipv4"
	for _, addr := range h.Addresses {
		if addr.AddrType == nmapAddrIPv4Field {
			return addr.Addr, nil
		}
	}

	return "", fmt.Errorf("GetIPv4: Host doesn't have IPv4")
}

func (h *Host) GetOpenPorts() []int32 {
	var openPorts []int32
	for _, port := range h.Ports {
		if port.State.State == "open" {
			openPorts = append(openPorts, int32(port.ID))
		}
	}
...
func oSNameToType(osName string) agent.OSType {
	if strings.Contains(osName, "linux") {
		return agent.OSType_OSTargetLinux
	} else if strings.Contains(osName, "windows") {
		return agent.OSType_OSTargetWindows
	} else if strings.Contains(osName, "mac os") {
		return agent.OSType_OSTargetMac
	}
...
type Port struct {
	...
	Service  NMapService `xml:"service" json:"service"`
...
type OS struct {
	PortsUsed    []PortUsed      `xml:"portused" json:"ports_used,omitempty"`
	Matches      []OSMatch       `xml:"osmatch" json:"os_matches,omitempty"`
	Fingerprints []OSFingerprint `xml:"osfingerprint" json:"os_fingerprints,omitempty"`
}
...
type NmapInfoModel struct {
	Key       string          `json:"key"`
	Address   *Address        `json:"address"`
	Hostname  string          `json:"hostname"`
	OSMatches []*NmapOSInfo   `json:"osMatches"`
	Ports     []*NmapPortInfo `json:"ports"`
	Edited    *EditModel      `json:"edited"`
}
...
func (o NmapOSInfo) OSFamily() family.OSFamily {
	return family.OSFamily(o.Family)
}
...
// ParseXMLToNMapRun takes a byte array of nmap xml data and unmarshals it into a
// Run struct.
func ParseXMLToNMapRun(content []byte) (*NMapRunEntity, error) {
	r := &NMapRunEntity{
		rawXML: content,
	}
...
func removeNull(hostname string) string {
	return isNull.ReplaceAllString(hostname, "")
}
